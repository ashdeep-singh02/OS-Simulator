...
// 	std::cout << (sim.GetCPU() == 5) << std::endl;
// 	// CHECK(sim.GetCPU() == 5);
// 	sim.DiskReadRequest(1, "abc");

// 	sim.DiskJobCompleted(0);
// 	sim.SimExit();
// 	std::cout << (sim.GetCPU() == 0) << std::endl;
// 	// CHECK(sim.GetCPU() == 0);
// 	auto q1{ sim.GetDiskQueue(1) };
// 	std::cout << (q1.size() == 0) << std::endl;
// 	// CHECK(q1.size() == 0);

// 	// std::cout << "247 " <<(sim.GetDisk(0).PID) << std::endl;
// 	std::cout << (sim.GetDisk(0).PID == 0) << std::endl;
// 	std::cout << (sim.GetDisk(1).PID == 5) << std::endl;
// 	// CHECK(sim.GetDisk(0).PID == 0);
// 	// CHECK(sim.GetDisk(1).PID == 5);
// 	std::cout << (sim.GetDiskQueue(0).size() == 0) << std::endl;
// 	std::cout << (sim.GetDiskQueue(1).size() == 0) << std::endl;
// 	// CHECK(sim.GetDiskQueue(0).size() == 0);
// 	// CHECK(sim.GetDiskQueue(1).size() == 0);

// 	auto mem{ sim.GetMemory() };
// 	std::cout << (mem.size() == 1) << std::endl;
// 	// CHECK(mem.size() == 1);
// 	std::cout << (ContMemoryItem(mem[0], 40, 10, 5)) << std::endl;
// 	// CHECK(ContMemoryItem(mem[0], 40, 10, 5));
